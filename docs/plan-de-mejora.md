...
- [2026-07-17] **Fase 0 completada** (chunks 0.1–0.5) — estado: `done` — rama `phase/0-reorg` mergeada a `develop`. Notas: se corrigieron 2 huecos del plan detectados en ejecución: (a) TODAS las rutas `./assets/` de `pages/*.html` deben pasar a `../assets/` (el plan decía "sin cambio" para CSS/JS de página); (b) los `url("../fonts/")` dentro de `core/base.css` deben pasar a `../../fonts/` al moverse el archivo a `core/`. Extra: fix de `minka-logo-negativo-2.png` (404 preexistente; no existía — se apuntó a `minka-logo-negativo-blanco.png`). Checkpoint C0 verificado en navegador local (sin 404, header inyectado, fuentes OK). Deploy de Pages pendiente de push.
- [2026-07-17] **Fase 1 completada** (chunks 1.1–1.11) — estado: `done` — rama `phase/1-bugs` mergeada a `develop`. Checkpoint C1: 12/12 verificados en navegador (login demo + remember → `localStorage.minka_session`; logout desde nav; guard redirect con `?redirect=`; publicar con `mode`+`notes`+dinámicos; editar título y pausar persisten tras recarga; QR pseudo-canvas como PNG real; búsqueda `?q=` + contador "N resultados"; geolocalización mock determinista por distrito; chat handoff `?thread=&owner=` con hilo dinámico, mensajes y respuesta mock persistidos; perfil público `?user=` readonly; eco-factores "Ropa y accesorios" correctos vía `constants.js`; claves huérfanas eliminadas). Desvíos documentados: `session.js`/`guard.js` se cargan SIN `defer` en el head (los page scripts van al final del body sin defer y correrían antes); el bloque muerto de `initSessionManagement` en `main.js` se eliminó completo; los hilos dinámicos de chat persisten en `minka_chat_dynamic_threads` (clave extra a migrar a Store en Fase 2).
- [2026-07-17] **Fase 2 completada** (chunks 2.1–2.8) — estado: `done` — rama `phase/2-persistence` mergeada a `develop`. Checkpoint C2: 8/8 verificados en navegador (dashboard estable tras recarga + CSV real; canje 250→50 persistido y coherente con home; participar en reto persiste y bloquea repetición; mensaje de chat → badge sube al instante → click en la notificación abre `chat.html?thread=`; perfil propio filtra por owner de sesión; perfil público readonly con distrito oculto ("Lima") y horarios; `localStorage` directo solo en excepciones justificadas — bloqueo de intentos en auth.js y timestamp de reauth; claves legacy con guion solo en la migración de store.js y el cleanup de session.js). Extras no previstos: `store.js` incluye migración automática de claves legacy; se cazaron y arreglaron 2 bugs PREEXISTENTES que C1 no había pisado: `notifications.js` corría antes que `i18n.js` (defer) y moría con ReferenceError dejando la página vacía (fix: init en DOMContentLoaded), y `profile.js` moría por TDZ de `updateProfileProgress` en `bindEvents` matando el render de "mis publicaciones" (fix: referencia lazy). Guard de robustez en dashboard para pestañas ocultas (rAF suspendido).
- [2026-07-18] **Fase 3 completada** (chunks 3.1–3.5) — estado: `done` — rama `phase/3-design-system` mergeada a `develop`. Checkpoint C3 verificado en navegador local: modal/badge/switch/form definidos SOLO en `core/components.css` (quedan `.badge-item/.badge-icon/.badge-name` en gamification.css — familia de medallas distinta al pill `.badge`, y overrides `body.high-contrast` legítimos en settings.css); estrellas de rating visibles con `--color-gray-200` nuevo; "Guardar preferencias" muestra toast; `rg "alert(|confirm(|prompt(" assets/scripts/pages/` = 0 (también se eliminaron los `onclick="alert()"` inline de auth.html); footer `.app-footer` en las 11 páginas de app con links válidos; breadcrumbs en detalle (título dinámico), publicar, perfil y chat; nav móvil 375px con `aria-expanded`/`aria-controls`, Escape y tap-fuera; sin JS (sin clase `html.js`) el nav queda inline y navegable y el hamburger se oculta. Piezas nuevas: `core/modal.js` (open/close por `aria-hidden`, focus trap, scroll lock, retorno de foco, delegación `data-action`/`data-close-modal`, y API `Modal.alert/confirm/prompt` promisificada con modal genérico) y `core/toast.js` (`Toast.show(msg, type)` con contenedor `aria-live`). Desvíos documentados: (a) el refactor `.card`/`card--<variant>` del Chunk 3.3 paso 2 se DIFIRIÓ (tocar 8 familias de cards en HTML+JS sin cobertura visual era alto riesgo; no está en los criterios de C3) — pendiente para Fase 4; (b) se estandarizó `.modal__content` (convención dominante en el markup real) en lugar del `.modal__dialog` del plan; (c) `modal.js`/`toast.js` se cargan SIN defer (mismo motivo que session.js en Fase 1); (d) el patrón hamburger del landing (`.header__menu-toggle` + main.js) se mantiene separado — el patrón único aplica a las 11 páginas de app. Extra: link muerto "Ver metodología" de detalle ahora abre el modal eco vía delegación; `Lighthouse` no se corrió (pendiente en C4, donde ya está previsto).
- [2026-07-18] **Fase 4 completada** (chunks 4.1–4.8) — estado: `done` — rama `phase/4-polish` mergeada a `develop`. Checkpoint C4 verificado en navegador + Lighthouse (Edge headless): **index perf 88 / a11y 96 / BP 100**; **auth a11y 100**. `home`/`detalle` no son auditables por Lighthouse sin sesión (guard.js redirige) — se auditó auth como proxy de app-page y los criterios de a11y se verificaron a mano (skip-link, aria, contraste por tokens). Verificado en navegador: empty state con doble CTA y `Store.seedDemo()` (8 items, categorías variadas, detalle navegable) puebla la grid sin recarga; doble click en Publicar crea exactamente 1 item con spinner y toast; validación on-blur con `aria-invalid`/`aria-describedby` + focus al primer error; toggle mostrar contraseña; contadores 0/80 y 0/300; búsqueda con debounce 300ms, chips con quitar individual + "Limpiar todo", URL↔filtros bidireccional y reproducible en frío; galería con ←/→ y "Foto N de M"; similares (≤3 misma categoría, oculto si no hay); compartir con Web Share → clipboard en cascada; favorito con `aria-pressed` persistido; dark mode automático por tokens (landing/about/auth fijadas a light con `html.force-light`); modales cerrados con `visibility: hidden` (fuera del tab order) sin romper el focus síncrono al abrir. Hallazgos de C4 no previstos y corregidos: (a) el embed de YouTube en index cargaba ~800KB de player al inicio — reemplazado por facade click-to-load (perf 64→88); (b) las fuentes TTF variables (2.6MB) eran el LCP — subset latino WOFF2 con fonttools (Inter 854→120KB; total ~400KB) con TTF de fallback; (c) el dark mode global rompía landing/footer (`--color-secondary-dark` se usa como texto Y fondo) — scoping con `:root:not(.force-light)` + fondo oscuro explícito en `.app-footer`; (d) verde de marca #2ecc71 como texto = 2.1:1 — nuevo token `--color-primary-ink: #1e8449` (4.72:1 AA) aplicado a `.btn-primary`, tabs activos y links de formulario. Desvíos documentados: critical CSS inline de index NO se hizo (sin CDN bloqueante el objetivo perf ≥85 se cumplió sin él); `auth-advanced.css` no se eliminó por purga sino que su contenido ya estaba fusionado en `auth.css` (se borró el duplicado); el dropdown de búsquedas recientes usa el `<datalist>` nativo existente (limitado a 5) en lugar de un dropdown custom; empty states de comunidad/gamification no aplican (tienen seed propio, lista nunca vacía); FA CDN se mantiene con carga diferida (`media="print"`); el refactor `.card`/`card--variant` diferido de F3 sigue pendiente (baja prioridad, sin criterio que lo cubra); sticky CTA móvil verificada por wiring (IntersectionObserver estándar) — el pane del navegador estaba `hidden` y no renderiza IO/rAF.
- [2026-07-18] **Fase 5 completada** (chunks 5.1–5.4) — estado: `done` — rama `phase/5-i18n` mergeada a `develop`. Checkpoint C5: 4/4 verificados en navegador. Diccionario: 347 claves parejas en es/en/qu, **0 claves faltantes** (auditoría automatizada con node contra todos los `data-i18n`/`I18n.t()` del código); 63 claves auth_*/search_* añadidas (5.1); 15 redeclaraciones muertas del bloque quechua eliminadas (ganaba la segunda copia — se conservó esa) y 4 claves huérfanas `home_*` borradas (5.3); 80 textos estáticos etiquetados con `data-i18n` en dashboard, gamification, chat, detalle, comunidad y breadcrumbs, con familia `common_*` para textos repetidos (Cancelar/Enviar/Confirmar/etc.) (5.2); i18n.js sin `console.log` y con registro silencioso de claves faltantes (`I18n.missingKeys`, log solo con `localStorage.minka_debug_i18n=1`) — `<html lang>` dinámico ya existía (5.4). Verificado en vivo: es→en→qu→es en settings/auth/detalle/gamification/dashboard/chat/comunidad/busqueda con `missingKeys=0` y consola limpia en todas; placeholders y breadcrumbs traducen. Nota: el quechua es traducción funcional para demo (variante sureña, consistente con el bloque existente) — revisar con hablante nativo antes de producción.
- [2026-07-18] **Checkpoint Final** — estado: `15/16 verificados localmente; deploy (CF1) pendiente de push`. Verificados en C0–C5 y re-chequeados: estructura objetivo (CF2), flujo end-to-end login→publicar→detalle→chat con persistencia (CF3, C1/C2), búsqueda ?q= + notificaciones + badge (CF4, C2/C4), dashboard estable + CSV (CF5), gamification y comunidad persistentes (CF6/7), perfil propio vs público (CF8), 0 `alert/confirm/prompt` (CF9), tokens y componentes únicos en core (CF10), skip-link/dark-mode/reduced-motion + Lighthouse a11y 96–100 (CF11), 0 hotlinks de imágenes + facades de YouTube en index y about + perf 88 (CF12), i18n completo con `<html lang>` dinámico y consola limpia (CF13), `rg "public/"` = 0 (CF14), claves legacy solo en el cleanup de session.js (CF15), UX completo: doble-submit bloqueado, URL de filtros, nav móvil, CTA fija, focus trap (CF16). **CF1 (deploy GitHub Pages) requiere push a remoto — no ejecutado por decisión de no pushear sin pedido explícito**; el workflow ya apunta a `path: .` desde Fase 0. develop mergeada a main localmente.
